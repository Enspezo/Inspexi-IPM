...
- `apps/api` — NestJS, ~60 modules, **149 Prisma-modellen** (API :3000, Swagger `/api/docs`)
...
**Online herstel van constateringen (PRD-14, module `client-repair`):** externen (anoniem, rapportnummer + postcode) en ingelogde klanten melden per constatering online herstel via het klantportaal (`{slug}.…:5174/herstel`). **Dubbele poort + plan-vlag**: add-on-entitlement **`ONLINE_HERSTEL`** → org-default **`Organization.onlineRepairDefault`** (alleen effect op nieuwe plannen, en alleen mét entitlement) → **`InspectionPlan.onlineRepairEnabled`**; aanzetten per plan vereist een gevuld, binnen de org uniek `referenceNumber` (= het rapportnummer waar de anonieme lookup op matcht, case-insensitief; postcode tegen `addressPostalCode` met fallback `Location.postalCode`). Eén sessiemodel **`RepairSession`** (ANONYMOUS/CLIENT_USER, 96-hex token, TTL 72u, lazy expiry) voor beide toegangsvormen; alle herstel-endpoints onder `/client/repair` lopen via de **`RepairSessionGuard`** (Bearer-sessietoken; COMPLETED-sessies blijven leesbaar voor bevestiging/PDF, mutaties asserteren ACTIVE). **Anti-enumeratie**: de lookup (throttle 5/min) draagt bewust GÉÉN `@RequiresFeature` en geeft bij élke mislukking dezelfde generieke 404-melding. **Claim = first-wins & atomisch**: `updateMany` flipt `Finding.statusCode` `open`→`resolved`; de verliezer krijgt een 409 en zijn invoer wordt bewaard als `FindingResolution` met status **`CONFLICT`** (nieuwe system-rows `REPORTED`/`CONFLICT` in `ResolutionStatusType`; `resolvedByClientUserId` is nu **nullable** voor anonieme sessies; `@@unique([findingId, repairSessionId])` vangt de dubbelklik-race binnen één sessie → P2002 = nette 400). Het rapportnummer is een **invariant** zolang de plan-vlag aan staat: ook een PATCH die alléén `referenceNumber` wijzigt valideert gevuld+uniek, en de org-default bij create zet de vlag alleen aan mét zo'n nummer. **Kritiek is configuratie**: `ClassificationOption.isCritical` (SUPERUSER-classificatiescherm) → gedenormaliseerd, server-owned `Finding.isCritical` (onderhouden in `findings.service` én de `/sync`-push; backfill: `apps/api/scripts/backfill-finding-is-critical.ts`); zodra géén kritieke findings meer open staan vuurt éénmalig (`criticalRepairNotifiedAt`, gereset bij heropenen van een kritieke finding) de **`HERINSPECTIE_VOORSTEL`**-dispatch. **Herstelverklaring** = `GeneratedDocument` met `DocumentType.HERSTELVERKLARING` uit een code-based Handlebars-template (foto's/logo als data-URI's — de Puppeteer-sandbox laat alleen `data:` toe), ondertekend via `DocumentSignature` met signer-role **`HERSTELLER`**; `GeneratedDocument.documentTemplateId` en `generatedBy` zijn hiervoor **nullable** geworden. Notificatietypes `HERSTEL_AFGEROND`/`HERSTEL_CONFLICT`/`HERINSPECTIE_VOORSTEL` (groep INSPECTIEPLANNEN) gaan naar de PM (`Project.projectManagerId` → `reviewerId` → `assignedTo`); e-mails (bevestiging + verklaring-PDF, conflict, herinspectie) via `RepairEmailService`. Herstelde constateringen tonen klant-zijdig nooit wie ze herstelde (staf ziet de invullergegevens wél). Seed (`SEED_DEMO=1`): demo-org krijgt de override + org-default, het demo-plan rapportnummer **`RAP-2026-001`** + vlag aan, C1 als kritieke optie (finding1 open-kritiek) en een afgeronde anonieme sessie met ondertekende herstelverklaring op finding2.

**AI-assistent backoffice (PRD-15, module `ai-agent`):** AI-agent-drawer in de portal-header voor backoffice-werk (opzoeken van relaties/aanvragen/taken, KvK/PDOK-lookups, web-search, en mutaties áchter een bevestigingskaart). 4 modellen (`AiConversation`/`AiMessage`/`AiPendingAction`/`AiUsageLog`), enum `AuditSource` + **`AuditLog.source`** (`HUMAN`|`AI`, default HUMAN — de audit-history toont een AI-badge; de mens blijft de acteur/`userId`). Drie gates op `/ai/*`: add-on-entitlement **`AI_AGENT`** (`@RequiresFeature`), per-org kill-switch **`Organization.aiAgentEnabled`** (+ **`aiAgentAllowedRoles`**, leeg = alle staf behalve INSPECTEUR) via de **`AiAgentAccessGuard`**, en de coarse `@Roles(...ALL_STAFF)` (bewust ruim zodat een per-org override INSPECTEUR kán toestaan); de UI probe't autoritatief via `GET /ai/access`. Antwoorden streamen als **SSE** (`token`/`tool`/`pending_actions`/`done`/`error`-events) — dat pad loopt búiten de exception-filter, dus fouten gaan daar als NL `error`-events (technische details alleen in de log). Tools delegeren naar de bestaande feature-services mét de ingelogde user (agent kan nooit meer dan de gebruiker zelf); bevestigde writes draaien in een geneste `requestContext` met `source: 'AI'`. Client: de **gedeelde `AnthropicClientService`** (nieuwe `streamMessage`-doorgeefluik) met **`ANTHROPIC_AGENT_MODEL`** (default `claude-sonnet-5`) en fair-use-metering (`AI_AGENT_MONTHLY_TOKEN_QUOTA`, 429 bij overschrijding). Org-settings-tab "Communicatie" beheert kill-switch + rollen (B-510: aanzetten vereist het entitlement). Zonder `ANTHROPIC_API_KEY` meldt de assistent netjes dat hij niet geconfigureerd is.

**Urenregistratie inspecteurs (PRD-16, module `time-tracking`, fase 1–4 afgerond):** add-on-entitlement **`URENREGISTRATIE`**; 3 modellen (`TimeEntry`/`Timesheet`/`InspectorLocationPing`) + `User.travelTrackingEnabled`/`travelTrackingConsentAt`. Vijf activiteiten (`VOORBEREIDING|UITVOERING|RAPPORTAGE|REISTIJD|OVERIG`), project verplicht behalve `OVERIG` — uitzondering: een `REIS_AUTO`-start mag tijdelijk zonder project (`needsProjectAssignment` + automatische todo-taak `TaskEntityType.TIME_ENTRY`, voltooid bij toewijzing; indienen blokkeert op open toewijzingen). **Eén timer tegelijk** per inspecteur, race-vrij via partial unique index **`imp_time_entries_running_user_key`** (`WHERE ended_at IS NULL AND is_deleted=false`; P2002 → 409) — leeft nét als de ltree-/AI-review-indexen **buiten het Prisma-schema**: `DROP INDEX`-regels handmatig strippen uit nieuwe migraties. Weekstaten = ISO-weken (lazy upsert op `@@unique([orgId,userId,year,weekNumber])`, weekbepaling in org-tijdzone via `isoWeekOf`), flow `CONCEPT→INGEDIEND→GOEDGEKEURD/AFGEWEZEN` (indienen alleen door de inspecteur zelf; beoordelen `MANAGEMENT_ROLES`; eigenaar mag muteren op CONCEPT/AFGEWEZEN, manager-correctie ook op INGEDIEND (`correctedById`), GOEDGEKEURD is bevroren). Endpoints: `/time-entries` (`start`/`stop`/`running` INSPECTEUR-only, CRUD; `clientId` idempotent voor de PWA-sync), `/timesheets` (+`:id/submit|approve|reject`, `export` = CSV `;`+BOM vóór de `:id`-route). Notificatiegroep **`UREN`** (WEEKSTAAT_*/TIMER_NACHTWAKER); audit op TimeEntry/Timesheet (pings bewust niet). Portal: **Uitvoering → Uren** (weekstaten-overzicht + detail met goedkeuren/afwijzen/corrigeren, CSV-export) en projectdetail-tab "Uren"; seed bij `SEED_DEMO=1` (override + weekstaten + lopende reistimer + 3 pings). Fase 2/3 server + portal: `/sync`-push-entiteit `timeEntries` (additief, eigenaar uit JWT, client-UUID = id+clientId, alleen afgeronde regels), nachtwaker- en ping-retentie-crons (`time-tracking.scheduler`), pings-endpoints (`POST /time-tracking/pings` = 200-noop zonder tracker/reistimer, `GET active`/`locations/:userId/latest`), opt-in `PATCH /users/me/travel-tracking`, en portal **Uitvoering → Uren → Nu actief** (Leaflet-kaartmodal, pin + bestemming + 200m-geofence; OSM-tiles). Fase 4: org-kill-switch **`Organization.travelTrackingEnabled`** (default aan; uit → opt-in geweigerd, pings noop, kaart 404 — ook via de by-slug-branding naar de PWA), kaart-toegangslogging (duurzame `InspectorLocationAccess`-rijen in `imp_audit_logs`, bewust buiten de portal-historie-UI), dashboard-widget "Nu actief" en een periode-filter op de CSV-export. PWA-kant (timers + reisdetectie) leeft in de `Inspexi-App`-repo (branch `claude/pwa-time-tracking-fase2`). Details: `docs/IMP_PRD_16_Urenregistratie-Locatietracking.md`.
...
- **Paginatie-basiscap = 200** (WP-B6/B-305): `BasePaginationQueryDto.limit` cap't op `@Max(200)` — hetzelfde contract als de portal (`limit: 200` voor dropdown-/lijstdata). Afwijken mag alleen bewust mét comment in de eigen DTO (hoger: locations 1000, projects/requests 500; lager: audit-logs 50, search 20, contextuele help 50); de parametrische e2e `test/pagination-limit.e2e-spec.ts` bewaakt dat elk gepagineerd endpoint `?limit=200` accepteert — nieuw gepagineerd endpoint dáár toevoegen. Portal-kant: query-fouten zijn nooit stil — globale `QueryCache.onError`-toast (opt-out `meta.suppressErrorToast`) + `<QueryErrorNotice>` bij dropdowns
...
**Online herstel (PRD-14):** `findingResolutionPhoto → findingResolution → repairSession` — de `repairSession` vóór `generatedDocument`, `clientUser` én `inspectionPlan` opruimen (FK's SetNull/Cascade); `FindingResolution.repairSessionId` is SetNull, dus de volgorde t.o.v. resoluties is soepel maar houd kinderen-eerst aan.
...
**AI-assistent (PRD-15):** `aiPendingAction`/`aiMessage` cascaderen op `aiConversation`; `aiUsageLog.conversationId` is een scalar (geen FK) maar ruim hem vóór `aiConversation` op; de conversation cascadeert op `user`/`organization`. Vóór de user-/org-cleanup:
```
aiPendingAction → aiMessage → aiUsageLog → aiConversation
```
