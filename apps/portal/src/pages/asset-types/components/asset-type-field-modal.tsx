// Aanmaak/bewerk-modal voor een asset-type-veld. Spiegelt lookup-edit-modal:
...
import { useCreateAssetTypeField, useUpdateAssetTypeField } from '../hooks/use-asset-types';
...
interface Props {
  ...
  field?: AssetTypeField | null;
}

export function AssetTypeFieldModal({ isOpen, onClose, assetTypeId, field }: Props) {
  const { showToast } = useToast();
  const createMutation = useCreateAssetTypeField(assetTypeId);
  const updateMutation = useUpdateAssetTypeField(assetTypeId);
  const isEdit = !!field;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Veld bewerken' : 'Veld toevoegen'}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Input label="Eenheid" placeholder="bijv. mm, kg" {...register('unit')} />
