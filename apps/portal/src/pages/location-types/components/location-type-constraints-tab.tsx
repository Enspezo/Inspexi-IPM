...
import {
  useLocationTypes,
  useLocationTypeConstraints,
  useSetLocationTypeConstraints,
...
export function LocationTypeConstraintsTab({ locationTypeId, canManage }: Props) {
  ...
  const parentOptions = [
    ...
    ...(allTypes ?? [])
      .filter((t: LocationTypeDefinition) => t.id !== locationTypeId)
      .map((t: LocationTypeDefinition) => ({ value: t.id, label: t.name })),
  ];
  ...
  return (
    <Card title="Toegestane ouder-types">
      <p className="mb-4 text-sm text-gray-600">
        Bepaal onder welke ouder-types dit locatie-type geplaatst mag worden. Gebruik
        &ldquo;Mag root zijn&rdquo; om het locatie-type op het hoogste niveau toe te staan.
        Markeer een ouder als &ldquo;verplicht&rdquo; wanneer dit locatie-type alleen daaronder mag bestaan.
      </p>

      {/* Huidige set als chips */}
      {working.length === 0 ? (
        <p className="text-sm text-gray-500">
          Geen beperkingen ingesteld — dit locatie-type mag overal geplaatst worden.
        </p>
      ) : (
