// Doel in apps/api: src/modules/assets/assets.service.ts
//
// Geport uit Inspexi-App. Beheer-conventies + Fase 1-schema:
//  - status (enum) → statusCode (lookup 'asset-status-types') via LookupService
//  - gedenormaliseerde orgId (overgenomen van het plan) → directe orgScope
//  - parent-constraint-validatie via AssetTypesService.validateParentConstraint
//  - boom (parentAssetId/treePath), reorder/move met circular-check, X-Device-ID
//  - soft-delete via deletedAt (incl. directe kinderen)
...
export class AssetsService {
  ...
  async findAllForOrganization(user: User, q: ListAssetsQueryDto) {
    ...
    return paginate(this.prisma.asset, {
      ...
      limit,
      select: {
        id: true, assetType: true, name: true, identifier: true,
        locationDescription: true, statusCode: true, notes: true,
        createdAt: true, updatedAt: true,
        inspectionPlan: {
          select: {
            id: true, projectName: true, normTypeCode: true, statusCode: true,
  ...
  async findAllByPlan(planId: string, user: User, options?: { parentId?: string; flat?: boolean }) {
    ...
    const assets = await this.prisma.asset.findMany({
      ...
      select: {
        id: true, parentAssetId: true, assetType: true, name: true, identifier: true,
        locationDescription: true, sortOrder: true, statusCode: true, technicalData: true,
  ...
  async create(planId: string, user: User, dto: CreateAssetDto, deviceId?: string) {
    ...
    const ok = await this.assetTypes.validateParentConstraint(dto.assetType, parentAssetTypeCode, user);
  ...
  async move(id: string, user: User, dto: MoveAssetDto) {
    ...
    if (dto.newParentId) {
      if (dto.newParentId === id) throw new BadRequestException('Een asset kan niet naar zichzelf verplaatst worden');
    ...
    const ok = await this.assetTypes.validateParentConstraint(asset.assetType, newParentTypeCode, user);
    if (!ok.valid) throw new BadRequestException(ok.message || 'Ongeldig ouder-type voor deze asset');
  ...
  private async isDescendantOf(assetId: string, ancestorId: string): Promise<boolean> {
    const asset = await this.prisma.asset.findFirst({
      where: { id: assetId, deletedAt: null }, select: { parentAssetId: true },
    });
  ...
  private mapAsset(a: any) {
    return {
      id: a.id, parentAssetId: a.parentAssetId, assetType: a.assetType, name: a.name,
      identifier: a.identifier, locationDescription: a.locationDescription, sortOrder: a.sortOrder,
      statusCode: a.statusCode, technicalData: a.technicalData, notes: a.notes,
