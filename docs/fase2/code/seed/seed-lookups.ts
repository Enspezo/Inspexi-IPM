// Doel in apps/api: prisma/seed-lookups.ts
//
// Seed van de 11 per-org lookup-tabellen uit Fase 1, als GLOBALE systeemdefaults
// (orgId = null, isSystem = true). Idempotent: deleteMany(systeem) + createMany.
// Org-overrides worden NIET geseed — die maakt een org zelf via /lookups.
//
// Gebruik in prisma/seed.ts:
//   import { seedLookups } from './seed-lookups';
//   await seedLookups(prisma);   // ergens in main(), volgorde maakt niet uit
//
// Labels zijn Nederlands; kleuren zijn HEX voor de status-badges in de portal/PWA.
