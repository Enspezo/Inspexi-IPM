// Doel in apps/api: src/modules/generated-documents/generated-documents.controller.ts
//
...
import {
  Controller, Get, Post, Patch, Delete, Param, Body, Res, ParseUUIDPipe, StreamableFile,
} from '@nestjs/common';
...
import {
  GenerateDocumentDto, UpdateGeneratedDocumentDto, RequestSignatureDto, SignDocumentDto, PublicSignDto,
} from './dto';
...
export class GeneratedDocumentsController {
  ...
  async generatePlan(@Param('planId', ParseUUIDPipe) planId: string, @CurrentUser() user: User, @Body() _dto: GenerateDocumentDto) {
  ...
  async generateReport(@Param('planId', ParseUUIDPipe) planId: string, @CurrentUser() user: User, @Body() _dto: GenerateDocumentDto) {
    return { success: true, data: await this.service.generateDocument(planId, DocumentType.REPORT, user) };
  }
  ...
  @Roles(...STAFF)
  async update(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() user: User, @Body() dto: UpdateGeneratedDocumentDto) {
  ...
  async preview(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() user: User, @Res({ passthrough: true }) res: Response): Promise<StreamableFile> {
  ...
  @Roles(...STAFF)
  async html(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() user: User, @Res({ passthrough: true }) res: Response) {
  ...
  @Roles(...STAFF)
  async requestSignature(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() user: User, @Body() dto: RequestSignatureDto) {
    return { success: true, data: await this.service.requestSignature(id, user, dto) };
  }

  @Post('generated-documents/:id/sign')
  @Roles(...STAFF)
  async sign(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() user: User, @Body() dto: SignDocumentDto) {
