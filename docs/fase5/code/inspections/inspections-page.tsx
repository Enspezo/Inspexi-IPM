// Doel in apps/portal: src/pages/inspections/inspections-page.tsx
//
// GOLDEN PATH overzichtspagina, 1-op-1 gespiegeld op pages/tasks/tasks-page.tsx
...
import { ErrorBox, Spinner, Table, Input, Select } from '@/components/ui';
...
const _canWrite = [Role.SUPERUSER, Role.ORG_ADMIN, Role.MANAGER, Role.BACKOFFICE, Role.WERKVOORBEREIDER];

function contactName(plan: InspectionPlan): string {
  const c = (plan as any).contact;
  if (!c) return '—';
...
export default function InspectionsPage() {
  ...
  const columns: ColumnDef<InspectionPlan>[] = [
    ...
    },
    {
      ...
      getFilterValue: (p) => (p as any).statusCode,
      render: (p) => <LookupBadge kind="plan-status-types" code={(p as any).statusCode} />,
    },
    {
      key: 'inspectionTypeCode', header: 'Type', groupable: true,
      render: (p) => <LookupBadge kind="inspection-types" code={(p as any).inspectionTypeCode} />,
    },
    {
      key: 'plannedDate', header: 'Gepland', filterable: true, filterType: 'date',
      sortable: true, sortKey: 'plannedDate', getFilterValue: (p) => (p as any).plannedDate,
      render: (p) => {
        const d = (p as any).plannedDate;
        return d ? <span className="text-xs text-gray-500">{new Date(d).toLocaleDateString('nl-NL')}</span>
  ...
  if (error) return <ErrorBox>Fout bij het laden van inspecties: {error.message}</ErrorBox>;
