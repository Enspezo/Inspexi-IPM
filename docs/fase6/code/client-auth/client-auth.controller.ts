// Doel in apps/api: src/modules/client-auth/client-auth.controller.ts
//
...
import {
  ClientLoginDto, ClientRegisterDto, ValidateMagicLinkDto,
  ClientForgotPasswordDto, ClientResetPasswordDto,
} from './dto';
...
export class ClientAuthController {
  ...
  @Throttle({ default: { limit: 5, ttl: 60000 } })
  async forgotPassword(@Body() _dto: ClientForgotPasswordDto) {
    // >>> PORT: Resend-mail met reset-token
    return { success: true };
  }

  @Post('reset-password')
  @Throttle({ default: { limit: 5, ttl: 60000 } })
  async resetPassword(@Body() _dto: ClientResetPasswordDto) {
    // >>> PORT: reset-token valideren + wachtwoord zetten
    return { success: true };
  }
  ...
  async me(@CurrentClientUser() user: CurrentClientUserData, @CurrentTenant('orgId') orgId: string | null) {
