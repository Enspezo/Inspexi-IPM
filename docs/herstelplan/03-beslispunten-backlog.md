...
| **A6** | **`reviewerId` via `/sync/push`.** Uit de whitelist halen is een contractversmalling. | WP-C3 (B-217) | (a) verwijderen — check eerst of de PWA het meestuurt; (b) rol-guarden + weigeren bij `reviewerId === user.id` | (a) als de PWA het niet gebruikt; planningsvelden horen via de portal |
...
| **B4** | **Enumeratiebescherming vs. branding-beschikbaarheid.** Achter één kantoor-NAT volstaan drie typefouten om iedereen 5 minuten een ongebrande loginpagina te geven. | B-511 §3 | Alleen echt onbekende slugs laten meetellen (inactieve orgs apart afhandelen) en de branding-lookup buiten de blokkade houden |
| **B5** | **`logoUrl` draagt twee betekenissen** — externe URL én interne storage key. `@IsUrl()` zou de uploadflow breken. | B-511 §5 (opgewaardeerd van S4) | Aparte `logoStorageKey`-kolom; beslis eerst of externe logo-URL's ondersteund blijven |
...
| **B7** | **"Uitnodigen voor één rapport" geeft toegang tot het hele klantdossier.** `grantPlanAccess` maakt naast de plan-grant een contact-brede `ClientAccess`. | B-411 punt 3 — **verdient een eigen bevinding**, staat nu verstopt als voetnoot | Autorisatie-ontwerpvraag. Als het antwoord "nee" is, is dat feature-werk aan het toegangsmodel |
