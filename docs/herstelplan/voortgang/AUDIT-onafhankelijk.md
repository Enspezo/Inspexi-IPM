...
| (extra) — ontbrekende CI-workflows | ✅ 29-07: het laatste structurele gat (geen `.github/workflows` in beide repo's) gedicht — Beheer PR #187 (build+unit-poort + aparte e2e-job met Postgres/seed/Puppeteer) en InspeXi #43 (build + typecheck + Vitest); branch-protection-advies in de PR-body's, lint als follow-up (in beide repo's niet werkend geconfigureerd) — zie logboek 29-07 |
